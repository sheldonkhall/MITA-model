...
t_out = np.linspace(0,tmax,601) # numpy vector of times at which to save to disk
...
#for run in [0]:
    ...
    EM_parameters.cond_rate = scale_param(cond_rate_, cond_rate, parameters[run,5])/100*EM_parameters.cond
    logfile.write('cond_rate: ' + str(scale_param(cond_rate_, cond_rate, parameters[run,5])/100*EM_parameters.cond) + '\n')
